...
/**
 * Debug function to test database connection and list all users
 * This helps troubleshoot database issues
...
export const debugDatabaseConnection = async () => {
  try {
    ...
    console.log('📈 Total documents found:', response.documents?.length || 0);
...
/**
 * Manual test function to sync user role with detailed logging
 * This function can be called manually to test the role sync process
 * @param {string} authUserId - User's Appwrite Auth ID
 * @param {Array} labels - User's labels
 * @param {string} email - User's email
 * @returns {Promise<Object>} - Test result
 */
export const manualTestRoleSync = async (authUserId, labels, email) => {
  try {
    console.log('🧪 MANUAL ROLE SYNC TEST');
    console.log('========================');
    console.log('📝 Auth User ID:', authUserId);
    ...
    const result = await syncUserRoleFromLabels(authUserId, labels, email);
